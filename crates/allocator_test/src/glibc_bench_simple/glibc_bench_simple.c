...
do_benchmark (malloc_args *args, char**arr)
{
  ...
  for (int j = 0; j < iters; j++)
    {
      for (int i = 0; i < n; i++) {
        arr[i] = glibc_bench_simple_malloc (size);
        for(int g = 0; g < size; g++) { arr[i][g] =(char)g; }
      }

      // free half in fifo order
      for (int i = 0; i < n/2; i++) {
	      glibc_bench_simple_free (arr[i], size);
      }
...
void
bench_simple (unsigned long size)
{
  ...
  glibc_bench_simple_free (arr, MAX_ALLOCS * sizeof (void*));
...
void glibc_bench_simple_test_start(CallBackMalloc _cb1,CallBackMallocAligned _cb2,CallBackFree _cb3) {
  cb1_glibc_bench_simple = _cb1;
  cb2_glibc_bench_simple = _cb2;
  cb3_glibc_bench_simple = _cb3;
  long size = 16;
